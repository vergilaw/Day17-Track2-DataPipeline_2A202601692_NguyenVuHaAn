...
| **Nguyên nhân** | Model `gold_training_set` cấu hình `materialized = 'incremental'` nhưng không khai báo `unique_key`, khiến dbt mặc định sinh ra câu lệnh `INSERT` thuần (chiến lược append). Do đó khi gặp bản ghi cập nhật `op = 'u'` hoặc khi chạy lại một phân vùng/Clear Task, dbt chèn thêm các hàng mới thay vì ghi đè theo `ticket_id`, làm số hàng tăng lũy kế và trùng lặp ticket. Ngoài ra DAG bật `catchup=True` và `max_active_runs` không giới hạn khiến các DAG run chạy song song ghi đồng thời làm race condition. |
...
| **Nguyên nhân** | Cơ chế lọc cận dưới `where event_date > (select max(event_date) from {{ this }})` của dbt incremental chỉ lấy dữ liệu mới hơn mốc ngày lớn nhất đã có trong kho. Khi một sự kiện xảy ra ngày 08-12 nhưng đến ngày 08-15 mới tới kho (Late-arriving data), tại ngày 08-15 mốc `max(event_date)` đã là 08-14, nên điều kiện so sánh loại bỏ vĩnh viễn sự kiện này khỏi bảng tổng hợp Gold. |
...
| **Nguyên nhân** | File `schema.yml` tắt Data Contract (`enforced: false`) khiến pipeline không có cơ chế chặn/bắt lỗi dữ liệu sai cấu trúc. Macro chuẩn hóa cũ dùng `try_cast` khiến các nhãn chuỗi hợp lệ (`urgent`, `high`,...) bị biến thành `NULL`, đồng thời lại ép kiểu số thành công và chấp nhận các giá trị lỗi (`0`, `5`, `-1`) vào luồng dữ liệu chính làm suy giảm chất lượng dữ liệu huấn luyện. |
